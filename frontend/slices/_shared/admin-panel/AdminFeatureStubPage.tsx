...
export function AdminFeatureStubPage({ segment }: { segment: string }) {
  ...
  if (!block) notFound();
  // PUBLIC-demo isolation: render the bare *BlockView (in-memory
  // useDefaultXBindings fallback) so showcase visitors don't share/stomp the
  // Convex-backed admin state. Non-demo (clones + owner) keep the Convex path.
  if (IS_DEMO) {
